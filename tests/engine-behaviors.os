...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьПереходНаИзвестнуюМетку");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьПереходНаМеткуВпереди");
...
Процедура ТестДолжен_ПроверитьПереходНаИзвестнуюМетку() Экспорт

	ПроверяемоеЗначение = "";
	Счетчик = 0;
	~ЗаранееОпределеннаяМетка:
		
		Счетчик = Счетчик + 1;
		ПроверяемоеЗначение = ПроверяемоеЗначение + Счетчик;
		Если Счетчик < 5 Тогда
			Перейти ~ЗаранееОпределеннаяМетка;
		КонецЕсли;

	юТест.ПроверитьРавенство(ПроверяемоеЗначение, "12345");

КонецПроцедуры

Процедура ТестДолжен_ПроверитьПереходНаМеткуВпереди() Экспорт
